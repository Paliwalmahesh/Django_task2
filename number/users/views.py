    ...
    if request.method == 'POST':
        ...
        if password1==password2:
            if User.objects.filter(username=username).exists():
                return render(request,'users/register.html',{'i':'user already exsist'})
            else:
            ...
            return render(request,'users/register.html',{'i':'paswwords are not same'})
    else:
...
def login(request):
    if request.method=='POST':
        ...
        if user is not None:
            ...
            return render(request,'users/login.html',{'i':'invalid username or password'})
    else:
